...
#ifdef USE_VULKAN
	// XR frame is started by RE_BeginXRFrame() before game logic runs
	tess.depthRange = DEPTH_RANGE_NORMAL;
...
#ifdef USE_VULKAN
	// TODO: Implement Vulkan color mask via pipeline dynamic state or specialization constant
#else
...
static const void *RB_SwapBuffers( const void *data ) {
	...
	// TODO: Screenshot support for VR would need to read from XR swapchain
	// and handle the different resolution
...
Switches rendering to/from the HUD buffer for in-world HUD sprite.
====================
*/
static const void* RB_HUDBuffer( const void* data ) {
	...
	// HUD mode 1 (in-world sprite): switch to HUD buffer render pass
	if ( cmd->start && !backEnd.isDrawingHUD ) {
		vk_begin_hud_render_pass( cmd->clear );
		if ( vk.renderPassIndex == RENDER_PASS_HUD ) {
			backEnd.isDrawingHUD = qtrue;
			// Force 2D setup on next draw so matrices are set for HUD buffer dimensions
			backEnd.projection2D = qfalse;
		}
	}
	else if ( !cmd->start && backEnd.isDrawingHUD ) {
