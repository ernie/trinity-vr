...
void R_InitImages( void ) {
	...
	// Create HUD buffer for VR HUD mode 1 (in-world sprite)
	...
	// Create virtual screen meshes (cylinder + floor quad)
	vk_create_virtual_screen_meshes();

	// Create virtual screen and floor grid pipelines
	vk_create_virtual_screen_pipelines();
#endif
}
...
Creates an image_t wrapper for the HUD buffer's VkImage/VkImageView.
The VkImage is owned externally by vk.xr, so isExternal is set to prevent
R_DeleteTextures() from destroying it.
===============
*/
image_t *R_CreateHUDImage( VkImage vkImage, VkImageView vkView, VkDescriptorSet descriptor,
                           int width, int height )
{
	image_t *image;
	long hash;
	const char *name = "*hudBuffer";
	int namelen = (int)strlen( name ) + 1;

	if ( tr.numImages == MAX_DRAWIMAGES ) {
		ri.Error( ERR_DROP, "R_CreateHUDImage: MAX_DRAWIMAGES hit" );
	}
	...
	image->flags = IMGFLAG_CLAMPTOEDGE;
	image->width = width;
	image->height = height;
	...
	image->isExternal = qtrue;  // VkImage owned by vk.xr.hudImage
