...
// Shared globals expected by SDL glimp code
...
static void InitOpenGL( void )
{
	...
	if ( glConfig.vidWidth == 0 )
	{
		...
		// Q3VR: r_renderScale and r_ext_supersample not supported
		// OpenXR controls VR render resolution, not these cvars
		...
		// Q3VR: Force software gamma for VR; hardware gamma only affects desktop monitor
		// Don't call GLimp_InitGamma as it would override this based on SDL window test
		glConfig.deviceSupportsGamma = qfalse;
		gls.deviceSupportsGamma = qfalse;
	...
	// Initialize VR session and renderer (creates XR session, swapchains)
...
void RB_TakeScreenshotBMP( int x, int y, int width, int height, const char *fileName, int clipboardOnly )
{
	...
	// Q3VR: clipboard screenshot not supported
	if ( !clipboardOnly ) {
...
static void R_Register( void )
{
	...
	ri.Cvar_CheckRange( r_intensity, 1, 255, qfalse );
	...
	r_roundImagesDown = ri.Cvar_Get ("r_roundImagesDown", "0", CVAR_ARCHIVE_ND | CVAR_LATCH );
	...
	//
	r_lodCurveError = ri.Cvar_Get( "r_lodCurveError", "8192", CVAR_ARCHIVE_ND );
	...
	ri.Cvar_CheckRange( r_gamma, 0.5f, 3, qfalse );
	...
	ri.Cvar_CheckRange( r_flareSize, 1, 40, qfalse );
	...
	ri.Cvar_CheckRange( r_flareCoeff, 0.1f, 99999, qfalse );  // Q3VR doesn't support open-ended ranges
	...
	r_mode = ri.Cvar_Get( "r_mode", "-2", CVAR_ARCHIVE_ND | CVAR_LATCH );
	r_fullscreen = ri.Cvar_Get( "r_fullscreen", "1", CVAR_ARCHIVE_ND );
	r_noborder = ri.Cvar_Get( "r_noborder", "0", CVAR_ARCHIVE_ND | CVAR_LATCH );
	...
	ri.Cvar_CheckRange( r_device, -2, 16, qtrue );  // 16 devices should be more than enough
	...
	ri.Cvar_CheckRange( r_hdrSoftKnee, 0.0, 2.0, qfalse );
	...
	ri.Cvar_CheckRange( r_renderHeight, 72, 4320, qtrue );  // 8K max
...
static void RE_Shutdown( refShutdownCode_t code ) {
	...
	if ( code != REF_KEEP_CONTEXT ) {
#ifdef USE_VULKAN
		// Destroy XR image views and framebuffers BEFORE destroying XR swapchains.
		// VKimp_Shutdown() calls xrDestroySwapchain() which invalidates the VkImages,
...
VR FUNCTIONS - stubs for initial compilation

These functions are required by Q3VR's refexport_t interface.
Full implementation will be added when integrating with OpenXR.
...
void RE_SetVRHeadsetParms( const float projectionMatrix[16],
	...
	// Store projection matrices in tr.vrParms
	Com_Memcpy( tr.vrParms.projection, projectionMatrix, sizeof(float) * 16 );
	Com_Memcpy( tr.vrParms.projectionEye[0], projectionEye0, sizeof(float) * 16 );
	Com_Memcpy( tr.vrParms.projectionEye[1], projectionEye1, sizeof(float) * 16 );

	// Copy mono projection (30° symmetric FOV) for HUD 3D models and virtual screen
	...
	// Vertical FOV is calculated using Q3A's formula: fov_y = atan2(height, width/tan(fov_x/2)) * 2
	{
		...
		tr.vrParms.menuProjection[10] = zNear / depth;
		tr.vrParms.menuProjection[14] = zFar * zNear / depth;
#else
		tr.vrParms.menuProjection[10] = -zFar / depth;
		tr.vrParms.menuProjection[14] = -zFar * zNear / depth;
#endif
...
void RE_ClearVRFramebuffer( int width, int height, qboolean isThirdPersonSpectator ) {
	...
	// The clear color is set in the render pass begin info (black by default, or blood red for spectator)
	(void)width;
	(void)height;
	(void)isThirdPersonSpectator;
}

void RE_SwapDesktopWindow( void ) {
	// All acquire/blit/present logic is in vk.c where the Vulkan function pointers are
...
void RE_WaitForRenderComplete( void ) {
	...
	result = vkWaitForFences( vk.device, 1,
		&vk.cmd->rendering_finished_fence,
		VK_TRUE, 1000000000 ); // 1 second timeout

	if ( result == VK_TIMEOUT ) {
		ri.Printf( PRINT_WARNING, "RE_WaitForRenderComplete: GPU fence timeout\n" );
	}

	vkResetFences( vk.device, 1, &vk.cmd->rendering_finished_fence );
...
qboolean RE_InitXRResources( void ) {
	// Pull swapchain info from VR layer and initialize XR resources
	// This is called from VR layer after XR swapchains are created
...
WRAPPER FUNCTIONS - adapt renderervk signatures to Q3VR's refexport_t

============================================================================
*/

static void RE_GetConfig_Wrapper( glconfig_t *config ) {
	// Quake3e returns const glconfig_t*, Q3VR expects void(glconfig_t*)
	Com_Memcpy( config, RE_GetConfig(), sizeof( glconfig_t ) );
}

static qboolean RE_VertexLighting_Wrapper( void ) {
	// Quake3e has void RE_VertexLighting(qboolean), Q3VR expects qboolean RE_VertexLighting(void)
	// Return current vertex lighting state from cvar
...
refexport_t *GetRefAPI ( int apiVersion, refimport_t *rimp ) {
	...
	re.GetConfig = RE_GetConfig_Wrapper;
	re.VertexLighting = RE_VertexLighting_Wrapper;
