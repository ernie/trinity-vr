...
#define MAX_TEXTURE_SIZE	8192 // must be less or equal to 32768
...
// Override renderercommon's 10-bit value with 12 bits for renderervk
#undef REFENTITYNUM_BITS
#undef REFENTITYNUM_MASK
#undef MAX_REFENTITIES
#undef REFENTITYNUM_WORLD
...
#endif
	qboolean	isHUD;			// true when rendering 3D content for the HUD buffer
} trRefdef_t;
...
typedef struct image_s {
	...
	// If true, VkImage is owned externally (e.g., HUD buffer, XR swapchain).
	// R_DeleteTextures() will NOT destroy the VkImage or VkImageView for this image.
	qboolean	isExternal;
#else
