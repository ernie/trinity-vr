...
For VR, this builds per-eye view matrices with IPD offset.
=================
*/
static void R_RotateForViewer( void )
{
	...
	// Each eye gets its own view matrix with IPD offset
	...
	for (int eye = 0; eye <= 2; ++eye)
	{
		...
		{
			// World model matrix (center, for culling and other operations)
			// convert from our coordinate system (looking down X)
			// to OpenGL's coordinate system (looking down -Z)
...
static void R_SetupProjectionZ( viewParms_t *dest )
{
	...
	// VR projection from OpenXR already has correct Z components (with reversed depth)
	if ( !tr.vrParms.valid ) {
	...
	if ( dest->portalView != PV_NONE )
	{
		...
		// VR: Compute per-eye mirror projections with oblique near-plane clipping
		// This is needed for proper stereo rendering of mirrors/portals
		if ( tr.vrParms.valid ) {
			for ( int eye = 0; eye < 2; eye++ ) {
				// Start with the regular per-eye projection
				Com_Memcpy( tr.vrParms.mirrorProjectionEye[eye], tr.vrParms.projectionEye[eye], sizeof(float) * 16 );

#ifdef USE_REVERSED_DEPTH
				// The per-eye projections from OpenXR are already in reversed depth format.
				// We need to temporarily convert back to standard depth for the oblique
				// clipping calculation, then convert the result back to reversed depth.
				//
				// Reversed depth (infinite): m[10] = 0, m[14] = near
				// Standard depth (infinite): m[10] = -1, m[14] = -near
				float stdM10 = tr.vrParms.projectionEye[eye][10] - 1.0f;  // 0 -> -1
				float stdM14 = -tr.vrParms.projectionEye[eye][14];        // near -> -near
#else
				float stdM10 = tr.vrParms.projectionEye[eye][10];
...
#ifdef USE_VULKAN
				tr.vrParms.mirrorProjectionEye[eye][10] = c[2];
#else
...
#ifdef USE_REVERSED_DEPTH
				tr.vrParms.mirrorProjectionEye[eye][2] = -tr.vrParms.mirrorProjectionEye[eye][2];
				tr.vrParms.mirrorProjectionEye[eye][6] = -tr.vrParms.mirrorProjectionEye[eye][6];
				tr.vrParms.mirrorProjectionEye[eye][10] = -(tr.vrParms.mirrorProjectionEye[eye][10] + 1.0);
				tr.vrParms.mirrorProjectionEye[eye][14] = -tr.vrParms.mirrorProjectionEye[eye][14];
#endif
			}
		}
	}
